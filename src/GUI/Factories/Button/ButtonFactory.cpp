    ...
    const GUI::ButtonConfig &conf)
{
    ...
    Component::render2dMapModels my_models({{"idle", std::make_shared<raylib::Rectangle>(position, conf.size)},
        {"hover", std::make_shared<raylib::Rectangle>(position, conf.size)},
        {"clicked", std::make_shared<raylib::Rectangle>(position, conf.size)},
        {"unavailable", std::make_shared<raylib::Rectangle>(position, conf.size)}});
