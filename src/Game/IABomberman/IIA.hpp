...
namespace IA
{
    ...
    template <typename TileType, typename Action> class IIA {
      public:
        virtual ~IIA(){};
        ...
        virtual void setIAAction(
            Action act, std::function<bool(std::vector<std::vector<TileType>> env, std::pair<size_t, size_t> pos)> func) = 0;
        virtual void setIAMovement(std::function<void(
                std::vector<std::vector<TileType>> env, std::pair<size_t, size_t> pos, std::queue<IA::Movement> &list)> func) = 0;
