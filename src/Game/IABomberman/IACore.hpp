...
namespace IA
{
    template <typename TileType, typename Action> class IACore : public IIA<TileType, Action> {
      public:
        ...
        void setIAMovement(std::function<void(
                std::vector<std::vector<TileType>> env, std::pair<size_t, size_t> pos, std::queue<IA::Movement> &list)> func);
